...
it('it initializes correctly', function () {
    ...
    $this->envStorage->shouldHaveReceived('read')->with($this->testBelichEnvPath)->once();
    $this->envParser->shouldHaveReceived('parse')->with('')->once();
    // $this->envEditor->setLines([]) would have been called on the real editor instance

    expect($this->envManager)->toBeInstanceOf(EnvManager::class);
    expect($this->backupManager)->toBeInstanceOf(BackupManager::class); // Corrected class name
});
...
test('it loads and parses env content', function () {
    ...
    $this->envStorage->shouldReceive('read')->with($this->testBelichEnvPath)->once()->andReturn($content);
    $this->envParser->shouldReceive('parse')->with($content)->once()->andReturn($parsedLines);
    // EnvEditor's setLines will be called with $parsedLines
...
test('it sets and gets values correctly', function () {
    ...
    $this->envFormatter->shouldReceive('format')->once()
...
test('it handles inline comments', function () {
    ...
    $this->envFormatter->shouldReceive('format')->once()
        ->with(Mockery::on(function ($lines) {
            return $lines[0]['key'] === 'COMMENTED_KEY' && $lines[0]['comment_inline'] === 'This is a comment';
        }))
        ->andReturn($expectedFormattedContent);
    $this->envStorage->shouldReceive('write')->with($this->testBelichEnvPath, $expectedFormattedContent)->once()->andReturn(true);
...
test('it handles block comments', function () {
    ...
    $this->envFormatter->shouldReceive('format')->once()
        ->with(Mockery::on(function ($lines) {
            return $lines[0]['key'] === 'BLOCK_KEY' && $lines[0]['comment_above'] === ['# First comment', '# Second comment'];
        }))
        ->andReturn($expectedFormattedContent);
    $this->envStorage->shouldReceive('write')->with($this->testBelichEnvPath, $expectedFormattedContent)->once()->andReturn(true);
...
test('it creates backups when saving if enabled', function () {
    ...
    $this->backupManager
        ...
        ->with($envPath) // Ensure it's called with the correct path
        ->andReturn(true);
...
test('it does not create backups when disabled', function () {
    ...
    $this->envStorage->shouldReceive('read')->with($this->testBelichEnvPath)->andReturn('')->byDefault(); // For the new instance's load
    $this->envParser->shouldReceive('parse')->with('')->andReturn([])->byDefault(); // For the new instance's load


    $this->backupManager->shouldReceive('create')->never();

    $envManager->set('NO_BACKUP_KEY', 'no_backup');
...
test('it handles non-existent env file on load', function () {
    ...
    $this->envStorage->shouldReceive('read')->with($this->testBelichEnvPath)->once()->andReturn(''); // Simulate file not existing or empty
    $this->envParser->shouldReceive('parse')->with('')->once()->andReturn([]);
    ...
    $this->envFormatter->shouldReceive('format')->with([])->once()->andReturn('');
...
test('it handles export statements correctly', function () {
    ...
    $this->backupManager->shouldReceive('create')->once()->andReturn(true); // For the second save
...
test('it cleans up empty lines after removal', function () {
    ...
    expect($newContent)->toContain("KEY1=value1".PHP_EOL.PHP_EOL."KEY3=value3");
});
...
test('it handles multiple consecutive saves', function () {
    ...
    $this->backupManager
        ...
        ->twice() // Expect backup for each save
        ->with($envPath)
        ->andReturn(true);

    // First save
    $this->envManager->set('FIRST_KEY', 'first_value');
    $this->envFormatter->shouldReceive('format')->once()->andReturn('FIRST_KEY="first_value"'.PHP_EOL);
    $this->envStorage->shouldReceive('write')->once()->andReturn(true);
    $this->envManager->save();

    // Second save
    $this->envManager->set('SECOND_KEY', 'second_value');
...
test('it preserves comments when updating values', function () {
    ...
    $this->envManager->set('KEY', 'new_value'); // EnvEditor should preserve comments if not overridden
...
test('it handles numeric values correctly', function () {
    $this->envManager->set('NUMBER_KEY', '12345'); // EnvManager expects string
    $this->backupManager->shouldReceive('create')->once()->andReturn(true);
